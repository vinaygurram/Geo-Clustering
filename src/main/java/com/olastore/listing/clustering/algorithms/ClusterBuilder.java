...
public class ClusterBuilder {
  ...
  public void reinitializeClusteringIndices() {
    try {
      ...
      FileUtils.copyInputStreamToFile(getClass().getClassLoader().getResourceAsStream((String) esConfig.get("geo_mappings_file_path")), file);
      JSONObject create_geo_response = esClient.createIndex((String) esConfig.get("geo_hash_index_name"), new FileEntity(file));
      logger.info("Creating geo mappings {}", create_geo_response.toString());

      FileUtils.copyInputStreamToFile(getClass().getClassLoader().getResourceAsStream((String) esConfig.get("cluster_mappings_file_path")),file);
      JSONObject create_cluster_response = esClient.createIndex((String) esConfig.get("clusters_index_name"), new FileEntity(file));
  ...
  private Set<String> initializePopularProductSet() {
    ...
    try {
      File file = new File("tmpFile");
      FileUtils.copyInputStreamToFile(getClass().getClassLoader().getResourceAsStream((String) clustersConfig.get("popular_products_file_path")), file);
